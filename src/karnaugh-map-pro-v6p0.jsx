...
/*
v5p25 circuit route audit fix
- Stagger final-gate input routes so different literal nets never visually merge
- Keep single-term multi-literal output connected through an explicit direct route
- Add safer route midpoint clamping between term outputs and final inputs
- Tighten SVG radius constants and route/dot rendering consistency
- Retain v5p24 net isolation fixes
*/
...
function StableCircuit({ plan, implMode="standard" }) {
  ...
  termsList.forEach((terms, gi) => {
    ...
    const routeSlot = gi - (termCount - 1) / 2;
    const rawMidX = finalX - 128 + routeSlot * 34;
    const maxMidX = finalInputX - 24;
    const midX = Math.min(maxMidX, Math.max(minMidX, rawMidX));
    ...
    if (isStandardPassThroughTerm) {
      ...
      if (termCount > 1 && literalNet) {
        const passMidX = Math.min(maxMidX, Math.max(inputX + 28, midX));
...
function ConstCircuit({ value }) {
  ...
  return <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:"100%",display:"block"}} preserveAspectRatio="xMidYMid meet">
    ...
    <circle cx={x2} cy={y} r={OR_BUBBLE_R} fill={C}/>
